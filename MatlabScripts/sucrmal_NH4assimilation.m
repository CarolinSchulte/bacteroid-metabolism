...
fluxBounds_o2 = [0:-0.1:-50];
 ...
 for i = fluxBounds_mal
    ...
    for j = fluxBounds_o2
        ...
        FBA = optimizeCbModel(model_gs,'max',0,0);
        if FBA.f > 0
            o2 = j;
            break
        end
...
for i = fluxBounds_mal
    ...
    for j = fluxBounds_o2
        ...
        try
            FBA = optimizeCbModel(model_wt,'max',0,0);
        catch
        end
        if FBA.f > 0
            o2 = j;
...
fluxBounds_o2 = [0:-0.1:-50];
 ...
 for i = fluxBounds_sucr
    ...
    for j = fluxBounds_o2
        ...
        FBA = optimizeCbModel(model_sucr_gs,'max',0,0);
        if FBA.f > 0
            o2 = j;
            break
        end
...
for i = fluxBounds_sucr
    ...
    for j = fluxBounds_o2
        ...
        try
            FBA = optimizeCbModel(model_sucr,'max',0,0);
        catch
        end
        if FBA.f > 0
            o2 = j;
