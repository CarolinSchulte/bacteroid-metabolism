...
model_dir = os.path.join(os.getcwd(), "models_bacteroid")
...
if __name__ == '__main__':
    start = time()
    AminoAcidList = ["Arg","Cys","GABA","Gln","Glu","GluOutTagged","Gly","His","Ile","Lys","Met","Phe","Pro","Ser","Thr","Trp","Tyr","Val"]
    for aminoacid in AminoAcidList:
        model_path = os.path.join(os.getcwd(),"models_bacteroid","bacteroid_"+ aminoacid+ "_ECM.xml")
        print(model_path)
        out_path = os.path.join(os.getcwd(),"results","bacteroid_"+ aminoacid+ "_ECMs.csv")
        input_file_path = os.path.join(os.getcwd(), "ECMinputs", "ECMs_"+aminoacid+".xlsx")
        colnames = ['Compound','Index','Input','Output']
        with open(input_file_path,mode='r',encoding='latin1',newline='\n'):
            inputfile = pandas.read_excel(input_file_path,names=colnames)
        #print(inputfile)
            index = inputfile.Index.tolist()
            inputList = inputfile.Input.tolist()
            outputList = inputfile.Output.tolist()
            inputscsv = ''
            outputscsv = ''
            for i in range(0,len(index)):
                if inputList[i] == 1:
                    inputscsv +=','+str(index[i])
                else:
                    outputscsv+= ','+str(index[i])
        ...
        parser.add_argument('--auto_direction', type=str2bool, default=False,
        ...
        if not args.auto_direction:
            set_inoutputs(inputscsv, outputscsv, network)
        ...
        if mpi_wrapper.is_first_process():
            try:
                np.savetxt(out_path, cone, delimiter=',', header=','.join(ids), comments='')
            except OverflowError:
                normalised = np.transpose(normalize_columns(np.transpose(cone)))
                np.savetxt(out_path, normalised, delimiter=',', header=','.join(ids), comments='')
